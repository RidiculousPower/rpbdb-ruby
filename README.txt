...
RPDB is an object-oriented database environment built on top of Oracle's Berkeley Database. 
...
== SUMMARY:
	...
	RPDB functions inside a database environment. The environment provides isolation for a 
	set of databases.
...
== INITIALIZING DATABASE ENVIRONMENT:
	...
	Any use of RPDB requires opening an environment. Multiple environments are supported, but
	most use cases will only require one.

	1. Declare an environment:
	
		environment = RPDB::Environment.new(	'name_to_identify_environment', 
...
== DECLARING A CLASS AS DATABASE OBJECT:
	...
	The best way to use RPDB with Ruby is with Database-Enabled Objects, which we will refer to
	as "Database Objects".
