...
void Init_rb_RPbdb_DatabaseJoinController()	{
	...
	rb_RPbdb_DatabaseJoinContainer	=	rb_define_module_under(	rb_RPbdb_Database, "Join" );

	rb_RPbdb_DatabaseJoinController		=	rb_define_class_under(	rb_RPbdb_DatabaseJoinContainer,				
																															"Controller",		
																															rb_cObject );
	
	rb_define_singleton_method(	rb_RPbdb_DatabaseJoinController, 	"new",													rb_RPbdb_DatabaseJoinController_new,											-1 	);
	rb_define_method(						rb_RPbdb_DatabaseJoinController, 	"initialize",										rb_RPbdb_DatabaseJoinController_initialize,										-1 	);
		
	rb_define_method(						rb_RPbdb_DatabaseJoinController, 	"settings_controller",					rb_RPbdb_DatabaseJoinController_settingsController,			0 	);
	rb_define_alias(						rb_RPbdb_DatabaseJoinController, 	"settings",											"settings_controller"	);
	rb_define_alias(						rb_RPbdb_DatabaseJoinController, 	"set",													"settings_controller"	);
	rb_define_alias(						rb_RPbdb_DatabaseJoinController, 	"set_to",												"settings_controller"	);
	rb_define_alias(						rb_RPbdb_DatabaseJoinController, 	"is_set_to",										"settings_controller"	);
	rb_define_method(						rb_RPbdb_DatabaseJoinController, 	"parent_environment",						rb_RPbdb_DatabaseJoinController_parentEnvironment,				0 	);
	rb_define_alias(						rb_RPbdb_DatabaseJoinController, 	"environment",									"parent_environment"	);
	rb_define_method(						rb_RPbdb_DatabaseJoinController, 	"parent_database",							rb_RPbdb_DatabaseJoinController_parentDatabase,					0 	);
	rb_define_alias(						rb_RPbdb_DatabaseJoinController, 	"database",											"parent_database"	);
	                  					
	rb_define_method(						rb_RPbdb_DatabaseJoinController, 	"join",													rb_RPbdb_DatabaseJoinController_join,										-1 	);
	rb_define_alias(						rb_RPbdb_DatabaseJoinController, 	"cursor",												"join"	);
	rb_define_method(						rb_RPbdb_DatabaseJoinController, 	"close_all_cursors",						rb_RPbdb_DatabaseJoinController_closeAllCursors,					-1 	);
...
VALUE rb_RPbdb_DatabaseJoinController_new(	int			argc,
	...
	VALUE	rb_join_controller	=	RUBY_RPBDB_DATABASE_JOIN_CONTROLLER( RPbdb_DatabaseJoinController_new( c_parent_database ) );
...
VALUE rb_RPbdb_DatabaseJoinController_initialize(	int			argc __attribute__ ((unused)),
...
*  settingsController  *
***************************/
VALUE rb_RPbdb_DatabaseJoinController_settingsController(	VALUE	rb_database_join_controller )	{
	...
	if ( ( rb_database_join_settings_controller = rb_iv_get(	rb_database_join_controller,
		...
		RPbdb_DatabaseJoinController*		c_database_join_controller;
		C_RPBDB_DATABASE_JOIN_CONTROLLER( rb_database_join_controller, c_database_join_controller );
	
		RPbdb_DatabaseJoinSettingsController*	c_database_join_settings_controller	=	RPbdb_DatabaseJoinController_settingsController( c_database_join_controller );

		rb_database_join_settings_controller	=	RUBY_RPBDB_DATABASE_JOIN_SETTINGS_CONTROLLER( c_database_join_settings_controller );
...
VALUE rb_RPbdb_DatabaseJoinController_parentEnvironment(	VALUE	rb_join_controller )	{
	...
	VALUE	rb_parent_database	=	rb_RPbdb_DatabaseJoinController_parentDatabase( rb_join_controller );
...
VALUE rb_RPbdb_DatabaseJoinController_join(	int			argc,
  ...
  VALUE	rb_join_cursor	=	rb_RPbdb_DatabaseJoinCursor_new(	c_passed_argc,
                                                          c_passed_args,
                                                          rb_join_controller );
...
VALUE rb_RPbdb_DatabaseJoinController_closeAllCursors( VALUE	rb_join_controller )	{
	...
	C_RPBDB_DATABASE_JOIN_CONTROLLER( rb_join_controller, c_join_controller );

	RPbdb_DatabaseJoinController_closeAllCursors( c_join_controller );
...
static int rb_RPbdb_DatabaseCursorController_internal_cursorsForEachHashDescriptor(	VALUE	rb_key_method_symbol,
																																										VALUE	rb_key_value,
...
VALUE rb_DatabaseJoinController_internal_cursorsForHashDescriptor(	VALUE	rb_database_join_controller,
  ...
  VALUE rb_primary_database = rb_RPbdb_DatabaseJoinController_parentDatabase( rb_database_join_controller );
	...
	rb_hash_foreach(	rb_hash_descriptor, 
										& rb_RPbdb_DatabaseCursorController_internal_cursorsForEachHashDescriptor,
										rb_passed_args );
    ...
    for ( c_which_cursor = 0 ; c_which_cursor < RARRAY_LEN( rb_cursor_array ) - 1 ; c_which_cursor++ )  {
      ...
      RPbdb_DatabaseCursor*	c_this_database_cursor;
      C_RPBDB_DATABASE_CURSOR( rb_this_database_cursor, c_this_database_cursor );
      RPbdb_DatabaseCursor_free( & c_this_database_cursor );
...
static int rb_RPbdb_DatabaseCursorController_internal_cursorsForEachHashDescriptor(	VALUE	rb_index,
																																										VALUE	rb_key,
																																										VALUE	rb_passed_args )	{
...
VALUE rb_RPbdb_DatabaseJoinController_internal_cursorForIndexAtKeyValue(	VALUE	rb_primary_database,
	...
	VALUE	rb_secondary_database	=	rb_RPbdb_Database_requireSecondaryDatabaseWithIndex(	rb_primary_database,
																																										rb_index_name );
	
	RPbdb_Database*		c_primary_database;
	C_RPBDB_DATABASE( rb_primary_database, c_primary_database );
  ...
  //  this is important, for instance, if we want :current to automatically unserialize
	...
	//	again, we call via rb_funcall so that the class determines whether or not to use RPbdb_DatabaseCursor or RPbdb_DatabaseObjectCursor
  ...
  //  since we may have opened cursors already we need to let our caller know that it should free them
	if ( rb_current == Qnil )   {		
		...
		RPbdb_DatabaseCursor_free( & c_database_cursor );
  ...
  VALUE rb_descriptor_hash    = Qnil;
	R_DefineAndParse( argc, args, rb_join_controller,
		R_DescribeParameterSet(
			R_ParameterSet(	R_Parameter(          R_MatchAncestorInstance(  rb_first_cursor,                    rb_RPbdb_DatabaseCursor ),
