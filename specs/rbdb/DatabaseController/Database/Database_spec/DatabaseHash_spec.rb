...
describe RPbdb::Database::Hash do
  ...
  it "can be created with a name and a parent environment" do
    RPbdb::Database::Hash.new( $database_name, @environment ).is_a?( RPbdb::Database::Hash ).should == true
  end

  # RPbdb::Database.new( name, environment_name ) 
  it "can be created with a name and a parent environment directory" do
    RPbdb::Database::Hash.new( $database_name, $environment_path ).is_a?( RPbdb::Database::Hash ).should == true  
  end

  # RPbdb::Database.new( name, database_controller ) 
  it "can be created with a name and a parent database controller" do
    RPbdb::Database::Hash.new( $database_name, @database_controller ).is_a?( RPbdb::Database::Hash ).should == true
  end

  # RPbdb::Database.new( name )  
  it "can be created with a name (using the default environment)" do
    RPbdb::Database::Hash.new( $database_name ).is_a?( RPbdb::Database::Hash ).should == true
  end
