...
describe RPbdb::Database do
  ...
  it "can be created with a name and a parent environment" do
    RPbdb::Database.new( $database_name, @environment ).is_a?( RPbdb::Database ).should == true
  end

  # RPbdb::Database.new( name, environment_name ) 
  it "can be created with a name and a parent environment directory" do
    RPbdb::Database.new( $database_name, $environment_path ).is_a?( RPbdb::Database ).should == true  
  end

  # RPbdb::Database.new( name, database_controller ) 
  it "can be created with a name and a parent database controller" do
    RPbdb::Database.new( $database_name, @database_controller ).is_a?( RPbdb::Database ).should == true
  end

  # RPbdb::Database.new( name )  
  it "can be created with a name (using the default environment)" do
    RPbdb::Database.new( $database_name ).is_a?( RPbdb::Database ).should == true
  end
  ...
  it "has a local settings controller" do
    ...
    database.settings.is_a?( RPbdb::Settings::Database ).should == true
  ...
  it "has a join controller" do
    @environment.database.new( $database_name ).join_controller.is_a?( RPbdb::Database::Join::Controller ).should == true
  end
  ...
  it "can return a cursor from its cursor controller" do
    @environment.database.new( $database_name ).cursor.is_a?( RPbdb::Database::Cursor ).should == true
  end
  ...
  it "can return an object cursor (which automatically handles serialization) from its cursor controller" do
    cursor = @environment.database.new( $database_name ).object_cursor.is_a?( RPbdb::Database::Cursor::ObjectCursor ).should == true
  end
  ...
  it "can be used in a block form" do
    RPbdb::Database.use( @environment, $database_name ) do
