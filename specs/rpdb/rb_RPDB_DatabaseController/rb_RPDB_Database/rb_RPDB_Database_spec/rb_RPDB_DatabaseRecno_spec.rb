...
describe Rbdb::Database::Recno do
  ...
  it "can be created with a name and a parent environment" do
    Rbdb::Database::Recno.new( $database_name, @environment ).is_a?( Rbdb::Database::Recno ).should == true
  end

  # Rbdb::Database.new( name, environment_name ) 
  it "can be created with a name and a parent environment directory" do
    Rbdb::Database::Recno.new( $database_name, $environment_path ).is_a?( Rbdb::Database::Recno ).should == true  
  end

  # Rbdb::Database.new( name, database_controller ) 
  it "can be created with a name and a parent database controller" do
    Rbdb::Database::Recno.new( $database_name, @database_controller ).is_a?( Rbdb::Database::Recno ).should == true
  end

  # Rbdb::Database.new( name )  
  it "can be created with a name (using the default environment)" do
    Rbdb::Database::Recno.new( $database_name ).is_a?( Rbdb::Database::Recno ).should == true
  end
